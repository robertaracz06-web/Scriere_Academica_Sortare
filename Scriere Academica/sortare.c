...
void SalveazaDateInFisier(int arr[], int n, const char* nume_fisier){
...
int main() {
    ...
    genereazaAleator(original, n);
    //genereazaSortat(original, n);
    //genereazaDescrescator(original, n);
    //genereazaAproapeSortat(original, n);
    //genereazaPlate(original, n);

    char nume_fisier[50];
    sprintf(nume_fisier, "date_test_%d.txt", n);
    SalveazaDateInFisier(original, n, nume_fisier);

    FILE *f_csv = fopen("rezultate.csv", "a");
    if(f_csv != NULL){
        fseek(f_csv, 0, SEEK_END);
        if(ftell(f_csv) == 0)
           fprintf(f_csv, "N,Algoritm,Timp_Sortare,Memorie_Extra_Bytes,Status\n");
    }
    
   printf("==============================================================\n");
    printf("Testam n = %d elemente (Timeout: %.0f sec)\n", n, timeout_limita_secunde);

    clock_t start, end;
    double timp_total, timp_mediu;
    char buffer_timp[50];
    int r; // Contor pentru repetarile dinamice
    ...
    } while (timp_total < 0.1 && !timeout_atins); // Rulează PÂNĂ CÂND adună măcar 0.1 secunde
    ...
    if (f_csv) { fprintf(f_csv, "%d,Bubble Sort,%s,%lld,%s\n", n, buffer_timp, memorie_extra_folosita, timeout_atins ? "TIMEOUT" : "OK"); fflush(f_csv); }
   ...
   memorie_extra_folosita = 0; timeout_atins = 0; timp_start_algoritm = clock();
    ...
    if (f_csv) { fprintf(f_csv, "%d,Selection Sort,%s,%lld,%s\n", n, buffer_timp, memorie_extra_folosita, timeout_atins ? "TIMEOUT" : "OK"); fflush(f_csv); }
    ...
    if (f_csv) { fprintf(f_csv, "%d,Insertion Sort,%s,%lld,%s\n", n, buffer_timp, memorie_extra_folosita, timeout_atins ? "TIMEOUT" : "OK"); fflush(f_csv); }
    ...
    if (f_csv) { fprintf(f_csv, "%d,Quick Sort,%s,Stack,OK\n", n, buffer_timp); fflush(f_csv); }
    ...
    if (f_csv) { fprintf(f_csv, "%d,Merge Sort,%s,%lld,OK\n", n, buffer_timp, memorie_extra_folosita / r); fflush(f_csv); }
